...
def get_current_mode() -> str:
    """Get current AutoResearch mode."""
    return json.dumps({"mode": hooks.read_current_mode()})


@mcp.tool()
def list_available_modes() -> str:
    """List all available modes."""
    rules = hooks.parse_mode_rules()
    return json.dumps({"modes": list(rules.get("modes", {}).keys())})


@mcp.tool()
def get_hooks_config() -> str:
    """Return current hooks configuration from .kilo/hooks/config.yaml."""
